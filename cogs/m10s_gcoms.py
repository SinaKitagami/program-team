...
class gcoms(commands.Cog):
    ...
    async def globaltester(self,ctx,uid,bl:bool=True):
        ...
        upf = self.bot.cursor.fetchone()
        if upf["gmod"] == True:
            self.bot.cursor.execute("UPDATE users SET galpha = ? WHERE id = ?", (int(bl),uid))
    ...
    async def globalmod(self,ctx,uid,bl:bool=True):
        print(f'{ctx.message.author.name}({ctx.message.guild.name})_'+ ctx.message.content )
        self.bot.cursor.execute("UPDATE users SET gmod = ? WHERE id = ?", (int(bl),uid))
    ...
    async def gconnect(self,ctx,name:str="main",dnf:bool=True):
        ...
        if ctx.author.permissions_in(ctx.channel).administrator == True or ctx.author.id == 404243934210949120:
            if ctx.channel.permissions_for(ctx.guild.me).manage_webhooks:
                ...
                if chs is None:
                    try:
                        ctg = self.bot.get_guild(560434525277126656).get_channel(582489567840436231)
                        cch = await ctg.create_text_channel(f'gch-{name}')
                        await cch.create_webhook(name="sina_global",avatar=None)
                        self.bot.cursor.execute("INSERT INTO globalchs(name,ids) VALUES(?,?)",(name,[ctx.channel.id,cch.id]))
                    except:
    ...
    async def globaldel(self,ctx,gmid:int,gchn:str):
        self.bot.cursor.execute("select * from globalchs where name = ?",(gchn,))
        ch = self.bot.cursor.fetchone()
        self.bot.cursor.execute("select * from users where id=?",(ctx.author.id,))
        upf = self.bot.cursor.fetchone()
        post=None

...
class music(commands.Cog):
            ...
            if vinfo.get("extractor","").startswith("youtube"):
                if vinfo.get("_type",None) == "playlist":
                    ...
                    for c in vinfo["entries"]:
                        tks.append(self.gpdate(f"https://www.youtube.com/watch?v={c['id']}",True))
    ...
    async def mplay(self,ctx,vl=1.0,lp=False):
        ...
        while self.bot.qu[str(ctx.guild.id)]:
            try:
                ...
                v=None
            if v:
                ctx.voice_client.play(discord.PCMVolumeTransformer(discord.FFmpegPCMAudio(f'musicfile/{self.bot.qu[str(ctx.guild.id)][0]["video_id"]}'),volume=v))
            else:
    ...
    async def playingmusic(self,ctx):
        if ctx.voice_client.is_playing():
            e = discord.Embed(title="再生中の曲",description=f'[{self.bot.qu[str(ctx.guild.id)][0]["video_title"]}]({self.bot.qu[str(ctx.guild.id)][0]["video_url"]})\nアップロードチャンネル:[{self.bot.qu[str(ctx.guild.id)][0]["video_up_name"]}]({self.bot.qu[str(ctx.guild.id)][0]["video_up_url"]})\nソース:{self.bot.qu[str(ctx.guild.id)][0]["video_source"]}')
            e.set_thumbnail(url=self.bot.qu[str(ctx.guild.id)][0]["video_thumbnail"])
    ...
    async def panel_update(self,ctx):
        ...
        if len(self.bot.qu[str(ctx.guild.id)])>1:
            ...
            ebd.add_field(name="次の曲:",value=f"[{self.bot.qu[str(ctx.guild.id)][0]['video_title']}]({self.bot.qu[str(ctx.guild.id)][0]['video_url']})(from {self.bot.qu[str(ctx.guild.id)][0]['video_source']})(スキップでキューから削除され、再生が止まります。)")
        else:
            ebd.add_field(name="次の曲:",value=f"再生終了")
        ebd.add_field(name="ループ:",value=self.bot.lp[str(ctx.guild.id)])
        try:
            ebd.add_field(name="ボリューム:",value=int(ctx.voice_client.source.volume*100))
        except:

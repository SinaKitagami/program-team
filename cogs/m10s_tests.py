...
class tests(commands.Cog):
    ...
    async def testA(self,ctx):
        e=discord.Embed(title="テスト",description="リアクションしてどうぞ",color=self.bot.ec)
        ...
        try:
            ...
            if str(r)=="1️⃣":
                e=discord.Embed(title="項目1",description="ブーストするといいことあるよ",color=self.bot.ec)
            elif str(r)=="2️⃣":
                e=discord.Embed(title="項目2",description="えーっと、書くこと知らないんだってよ",color=self.bot.ec)
            elif str(r)=="3️⃣":
                e=discord.Embed(title="項目3",description="まあ、これ、あくまで例示だしいいよね？",color=self.bot.ec)
            else:
                e=discord.Embed(title="項目?",description="このリアクションには、何もないよ！",color=self.bot.ec)
